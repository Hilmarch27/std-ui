...
function parseAsDate(timestamp: number | string | undefined): Date | undefined {
  ...
  const date = new Date(timestamp) // ISO string juga valid di sini
...
interface ClientTableDateFilterProps<TData> {
  column: Column<TData, unknown>
  title?: string
  multiple?: boolean
}

export function ClientTableDateFilter<TData>({ column, title, multiple }: ClientTableDateFilterProps<TData>) {
  ...
  const onSelect = React.useCallback(
    (date: Date | DateRange | undefined) => {
      ...
      if (multiple && !('getTime' in date)) {
        ...
        column.setFilterValue(date.toISOString())
      }
    },
    [column, multiple]
  )
